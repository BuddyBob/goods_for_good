...
const Items = ({ type }) => {
  ...
  return (
    <>
      ...
      <div className={`d-flex align-content-start flex-wrap   display: flex;
  flex-wrap: wrap;
  justify-content: center; /* Center items horizontally */
  align-items: center; ${popup ? 'blur-background' : ''}`}>
        {map && (
          <MapLoader
        ...
        {items.map((item, index) => (
          <div className="p-2">
            ...
            )}
            {
              ...
              type == "volunteer" && item.status === "pre_delivery" && (
                <button
                  ...
                  onClick={() => beginDelivery(item, index)}
                >
                  Begin Delivery
                </button>
              )
            ...
            <div key={itemIds[index]} className="card-item col mb-5">
              <img
              ...
              <div className="card-body">
                ...
                {item.status == "pre_delivery" && type != "volunteer" && (
                  <p className="card-text">
                    Volunteer {item.volunteer_delivering}
                  </p>
                )}

                {type === "donee" ? (
                  <p className="card-text">
                    <strong onClick={(e) => handleGoogleMapsLink(e, item.donee_location)}>
                    Pickup
                    </strong>{" "}
