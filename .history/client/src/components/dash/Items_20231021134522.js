...
const Items = ({type}) => {
  ...
  const confirmDelivery = (item, index) => {setItemData({"item":item, "index":index}); setPopup(true)}

  const onClose = () => {setPopup(false); window.location.reload();}


  return (
    <div className="d-flex align-content-start flex-wrap" >
      {popup &&
        <ConfirmPopup props={itemData} onClose={onClose}/>
      }
                ...
                <div className="card-body">
                  ...
                  {type === 'donee' ? (
                    <p className="card-text">
                      <strong onClick={(e) => handleGoogleMapsLink(e, item.donee_location)}>Pickup</strong> <SlLocationPin/> {item.donee_location}
                    </p>
                  ) : type === "donor" ? (
                    <p className="card-text">
                      <strong onClick={(e) => handleGoogleMapsLink(e, item.location)} >Item Drop Off </strong> <SlLocationPin/> {item.location}
                    </p>
                  ) : type === "volunteer" ? (
                    <p
                  ...
                  {/* volunteer confirm delivery successful */}
                  {
                    type === "volunteer" && item.status === "en_route" && (
                      <button className="btn btn-primary mt-2" onClick={() => confirmDelivery(item, index)}>Confirm Delivery</button>
                    )
                  }
