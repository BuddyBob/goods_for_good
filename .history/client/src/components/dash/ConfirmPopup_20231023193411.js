...
const ConfirmPopup = ({props, onClose}) => {
      ...
      directionsService.route(
        {
          ...
          destination: item.donee_location,
  ...
  const confirmDelivery = async (e) => {
    e.preventDefault();
    try {
      const duration = await returnDuration(props.item);
      console.log(duration); // Now you can use the duration
      // Continue with your code here, knowing that the duration is set.
    } catch (error) {
      console.error(error);
