...
const ConfirmPopup = ({props, onClose}) => {
  ...
  function parseDurationToMinutes(durationText) {
    ...
    for (let i = 0; i < parts.length; i++) {
      if (parts[i] === 'hour' || parts[i] === 'hours') {minutes += parseInt(parts[i - 1]) * 60;} 
      else if (parts[i] === 'min' || parts[i] === 'mins') {minutes += parseInt(parts[i - 1]);}
    }
    console.log(minutes)
    return minutes;
  }
  ...
  const confirmDelivery = async (e) => {
    ...
    let dur; // Declare dur variable
  
    try {
      dur = await returnDuration(props.item); 
  
      // Proceed with the fetch request after dur has been successfully retrieved
      const response = await fetch('http://127.0.1:4000/api/main/confirm-delivery', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          dur: duration // Use the retrieved dur
        })
      });
  ...
  return (
    <div className="container pop-container1">
      ...
      <div className="center-content">
        <AiOutlineClose className="add-popup-close2" onClick={() => onClose("exit")} />
