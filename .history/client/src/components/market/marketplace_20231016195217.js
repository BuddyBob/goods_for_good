              ...
              <div className="card-item mb-4" onClick={() => showPopup(itemIds[index],item)}>
                ...
                <div className="card-body">
                  ...
                  { item.status == "available" ? (
                    ...
                    <p className="card-text"><SlLocationPin onClick={(e) => handleGoogleMapsLink(e, item.donee_location)}/> {item.location}</p>
                  ) : (
                    <p
                      className={`card-text ${hovered ? 'hover-effect' : ''}`}
                      onClick={(e) => handleGoogleMapsDirLink(e, item.location, item.donee_location, index)}
                      onMouseEnter={() => {
                        setHovered(true);
                        setShowTooltip(true);
                      }}
