...
const Marketplace = () => {
  ...
  const noItems = () => toast("There are currently no items present")
  ...
  useEffect(() => {
    fetch('http://127.0.0.1:4000/api/main/get-all-items', {
      ...
      .then((data) => {
        console.log(data)
        setItems(data.items.flatMap(item => item).slice().reverse());
        setItemIds(data.item_ids.slice().reverse());
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  return (
    <>
      ...
      {addPopup && (<MarketPopup style={{ top: popupPosition.top}} onClose={() => setAddPopup(false)} itemId={selectedItemId} item={selectedItem} />)}
      </div>
          ...
          <div className="d-flex align-content-start flex-wrap item-container">
            {items.map((item, index) => (
              ...
              <div className="p-3" key={itemIds[index]}> 
                ...
                <div className="card-item mb-4" onClick={(e) => showPopup(itemIds[index], item, e)}>
