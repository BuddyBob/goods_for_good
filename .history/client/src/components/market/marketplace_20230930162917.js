      ...
      {addPopup && (
        ...
        <div className="row cards-item-row">
          {items.map((item, index) => (
            <div className="col-sm" key={itemIds[index]}> {/* Moved key to the outer div */}
            <div className="status">
              ...
              <div className="card-item" onClick={() => showPopup(itemIds[index], item.title, item.description, item.location, item.status, item.owner)}>
                <div style={{ "height": "18rem" }}>
                  <img src={`http://127.0.0.1:4000/api/main/get-image/${itemIds[index]}`} className="card-img-top img-fluid" style={{ "height": "100%", "max-width": "18rem", "min-width": "16rem"}} alt="Item" />
                </div>
