...
const Marketplace = () => {
  ...
  const showPopup = (itemId, title, description, location, status, owner) => {
    if (localStorage.getItem('userType') == 'donee' || localStorage.getItem('userType') == 'volunteer'){
      if (status == 'available'){
        setAddPopup(true);
            ...
            <div className="status">
              ...
              <div className="card-item" onClick={() => showPopup(itemIds[index], item.title, item.description, item.location, item.status, item.owner)}>
                <div style={{ "height": "18rem" }}>
                  <img src={`http://127.0.0.1:4000/api/main/get-image/${itemIds[index]}`} className="card-img-top img-fluid" style={{ "height": "100%", "width": "18rem" }} alt="Item" />
                </div>
